...
roomStore.getTeamScore = (teamId) => {
	return get(roomStore).teams.find((team) => team.id === teamId).score || 0
}

roomStore.incrementTeamScore = (teamId) => {
    // console.log("incrementTeamScore", teamId)
    // console.log("team", get(roomStore).teams.find((team) => team.id === teamId))
	get(roomStore).teams.find((team) => team.id === teamId).score++
}
