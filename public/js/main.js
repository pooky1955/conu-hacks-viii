...
const YBound = [-1, 1]
const ZBound = [0, 1]
const newXBound = [-0.018, 0.018]
const newYBound = [0.01, 0.02]
const newZBound = [-0.017, -0.018]
...
class Box {
	...
	setup(world) {
		...
		mesh.rotation.set(this.rotation[0] * ToRad, this.rotation[1] * ToRad, this.rotation[2] * ToRad)
		mesh.castShadow = true
		mesh.receiveShadow = true
		this.mesh = mesh
		this.config = {
...
class Sphere {
	...
	reset() {
		// setTimeout(() => {
		ball.body.resetPosition(0, 100, 150)
		// }, 900)
	}
...
function customLoop() {
	...
	if (!!hand) {
		if (true) {
			...
			if (handTrackerTimeout === 0 && l1norm(handtracker.velocity) > 0.7) {
				if (realImpulse.z < 0) {
